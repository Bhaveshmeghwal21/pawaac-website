...
describe("resolveActiveNavItem", () => {
  ...
  it("returns 'company' for each of the three Company_Menu-linked routes (About Us, Careers, Contact Us)", () => {
    expect(resolveActiveNavItem("/company")).toBe("company");
  ...
  // Feature: pawaac-design-language-evolution, Property 14: For any current route drawn from the set of all Pawaac_Site routes, the Navigation renders the active-item indicator under exactly one of the 4 primary items — Product, Autonomy, Resources, or Company — if the current route is /product, /autonomy, one of the three Resources_Menu-linked internal routes (/designer, /news, /commitments, all of which resolve to the Resources indicator), or one of the three Company_Menu-linked routes (/company, /careers, /contact, all of which resolve to the Company indicator), or under none of the 4 items if the current route matches none of those (i.e. Homepage only).
